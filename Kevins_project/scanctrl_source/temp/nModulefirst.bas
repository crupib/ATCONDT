...
TYPE FociRay
     ...
     i_yIndexEnd AS INTEGER     'number of Y-Indexes to last index


     r_xIndexIncFixed AS DOUBLE         'X-Index Increment set by user

     r_yIndexInc AS DOUBLE         'Y-Index increment for circ raster, ax or circ beam
     ...
     xOffset AS DOUBLE           'X-offset distance of transducer for circ beam
     ...
     RasterAxial AS LONG         'true or false: If false then circ scan direction
...
TYPE ScanSegment
   ...
   PathsGen AS LONG
   xMtrSeg(2000) AS DOUBLE
   yMtrSeg(2000) AS DOUBLE
   zMtrSeg(2000) AS DOUBLE
   xImgSeg(2000) AS DOUBLE
   yImgSeg(2000) AS DOUBLE
   xySegLn(2000) AS DOUBLE
...
SUB GenerateModel(BYVAL RayPtr AS DWORD)
    ...
    ELSE  ' no overlap!
       ...
       'DON'T want it to zip a full 360 degress back to 0, YIKES!!!!
...
SUB GenAxialSegments (BYVAL RayPtr AS DWORD, BYVAL sPathPtr AS DWORD)
    ...
    LOCAL Ray AS FociRay POINTER
    LOCAL sPath AS ScanSegment POINTER
    ...
    @sPath.PathsGen = 0  'tracks number of paths generated
    ...
    IF @Ray.RasterAxial THEN  'raster in Axial direction
        ...
        DO
             ...
             @sPath.xMtrSeg(PathCtr) = xPos2# - xPos1#
             @sPath.yMtrSeg(PathCtr) = yPos2# - yPos1#
             @sPath.zMtrSeg(PathCtr) = zPos2# - zPos1#
             @sPath.xImgSeg(PathCtr) = xImgPos2# - xImgPos1#
             @sPath.yImgSeg(PathCtr) = yImgPos2# - yImgPos1#

             @sPath.xySegLn(PathCtr) = GetSegLen (@sPath.xMtrSeg(PathCtr), @sPath.yMtrSeg(PathCtr))

             xPos1# = xPos2# : yPos1# = yPos2# : zPos1# = zPos2# : xImgPos1# = xImgPos2# : yImgPos1# = yImgPos2#

             'PRINT "Path Segment Length: "; PathCtr; @sPath.xySegLn(PathCtr) 'segment length
   ...
   ELSE   'Raster in the circ direction
        ...
        DO
            ...
            'PRINT "Path Segment Length: "; PathCtr; @sPath.xySegLn(PathCtr)
...
SUB GenCircSegments (BYVAL RayPtr AS DWORD, BYVAL sPathPtr AS DWORD)
    ...
    Ray = RayPtr       'set pointer
    sPath = sPathPtr   'set pointer
    ...
    IF @Ray.skewDegs < 0 THEN
       ...
       @Ray.xOffset = -(ABS(@Ray.xOffset))           'xOffset and skewDegs are negative while skewrads is positive
    ELSE
       @Ray.skewRads = -(DegToRads(@Ray.skewDegs))     'set to negative
       @Ray.xOffset = ABS(@Ray.xOffset)             'xOffset and skewDegs are positive while skewrads is negative
    END IF


    'below set for standalone testing
    '@Ray.r_yBegin = -(@uSCN.weldWidth + (@uSCN.weldHaz*2))
    '@Ray.r_yEnd  =  @Ray.r_yBegin + @uSCN.weldWidth + (@uSCN.weldHaz*2)


    'Scan start position
    xPos1# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin) * COS(@Ray.eNormAngRads(0)) + @Ray.eOriginX(0) + (@Ray.xOffset * COS(@Ray.eRotAngRads(0) + @Ray.skewRads))
    yPos1# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin) * SIN(@Ray.eNormAngRads(0))+ (@Ray.xOffset * SIN(@Ray.eRotAngRads(0) + @Ray.skewRads))
    ...
    IF @Ray.RasterAxial THEN  'raster in Axial direction
        ...
        pathbufflen = (@Ray.i_xIndexEnd + 1) * 2
        ...
        DO
             ...
             'get path target position - stroke away from weld
             xPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yEnd) * COS(@Ray.eNormAngRads(xIndexCtr)) + @Ray.eOriginX(xIndexCtr) + (@Ray.xOffset * COS(@Ray.eRotAngRads(xIndexCtr)+ @Ray.skewRads))
             yPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yEnd) * SIN(@Ray.eNormAngRads(xIndexCtr)) + (@Ray.xOffset * SIN(@Ray.eRotAngRads(xIndexCtr) + @Ray.skewRads))
       ...
       '     PRINT "Index CTR : "; IndexCtr; RadsToDeg(@Ray.eRotAngRads(IndexCtr)); @Ray.skewRads
             ...
             'get path target position - stroke to weld
             xPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yBegin) * COS(@Ray.eNormAngRads(xIndexCtr)) + @Ray.eOriginX(xIndexCtr)+ (@Ray.xOffset * COS(@Ray.eRotAngRads(xIndexCtr)+ @Ray.skewRads))
             yPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yBegin) * SIN(@Ray.eNormAngRads(xIndexCtr)) + (@Ray.xOffset * SIN(@Ray.eRotAngRads(xIndexCtr) + @Ray.skewRads))
             zPos2# = RadsToDeg( (@Ray.eRotAngRads(xIndexCtr)+ @Ray.skewRads) )
             xImgPos2# = @Ray.eArcTotal(xIndexCtr)
             yImgPos2# = @Ray.r_yBegin
    ...
    ELSE   'Raster in the circ direction
        ...
        'below values used for standalone testing
        '@Ray.r_yBegin = -(@Ray.weldWidth + (@Ray.weldHaz))
        '@Ray.r_yEnd  =  @Ray.r_yBegin + @uSCN.weldWidth + (@uSCN.weldHaz*2)
...
'        pathbufflen% = ((xIndexEnd#/indexInc#) + 2) * (@Ray.i_xIndexEnd + 1)
        ...
        DO
            ...
            FOR xIndexCtr = 1 TO @Ray.i_xIndexEnd '@ start position: drive positive circ direction to next path coordinate
                ...
                xPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yBegin + index#) * COS(@Ray.eNormAngRads(xIndexCtr)) + @Ray.eOriginX(xIndexCtr) + (@Ray.XOffset * COS(@Ray.eRotAngRads(xIndexCtr)+ @Ray.skewRads))
                yPos2# =  (@Ray.eRadiusNorm(xIndexCtr) + @Ray.r_yBegin + index#) * SIN(@Ray.eNormAngRads(xIndexCtr)) + (@Ray.XOffset * SIN(@Ray.eRotAngRads(xIndexCtr) + @Ray.skewRads))
                zPos2# = RadsToDeg( (@Ray.eRotAngRads(xIndexCtr) + @Ray.skewRads#) )
                xImgPos2# = @Ray.eArcTotal(xIndexCtr)
                yImgPos2# = @Ray.r_yBegin + index#
         ...
         '       PRINT "Path Segment Length: "; PathCtr; @sPath.xySegLn(PathCtr) 'segment length
            ...
            xPos1# =  (@Ray.eRadiusNorm(@Ray.i_xIndexEnd) + @Ray.r_yBegin + index#) * COS(@Ray.eNormAngRads(@Ray.i_xIndexEnd)) + @Ray.eOriginX(@Ray.i_xIndexEnd) + (@Ray.XOffset * COS(@Ray.eRotAngRads(@Ray.i_xIndexEnd)+ @Ray.skewRads))
            yPos1# =  (@Ray.eRadiusNorm(@Ray.i_xIndexEnd) + @Ray.r_yBegin + index#) * SIN(@Ray.eNormAngRads(@Ray.i_xIndexEnd)) + (@Ray.XOffset * SIN(@Ray.eRotAngRads(@Ray.i_xIndexEnd) + @Ray.skewRads))
            ...
            xPos2# =  (@Ray.eRadiusNorm(@Ray.i_xIndexEnd) + @Ray.r_yBegin + index#) * COS(@Ray.eNormAngRads(@Ray.i_xIndexEnd)) + @Ray.eOriginX(@Ray.i_xIndexEnd) + (@Ray.xOffset * COS(@Ray.eRotAngRads(@Ray.i_xIndexEnd)+ @Ray.skewRads))
            yPos2# =  (@Ray.eRadiusNorm(@Ray.i_xIndexEnd) + @Ray.r_yBegin + index#) * SIN(@Ray.eNormAngRads(@Ray.i_xIndexEnd)) + (@Ray.xOffset * SIN(@Ray.eRotAngRads(@Ray.i_xIndexEnd) + @Ray.skewRads))
        ...
        '        PRINT "Path Segment Length: "; PathCtr; @sPath.xySegLn(PathCtr)
            ...
            xPos1# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin + index#) * COS(@Ray.eNormAngRads(0)) + @Ray.eOriginX(0) + (@Ray.xOffset * COS(@Ray.eRotAngRads(0) + @Ray.skewRads))
            yPos1# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin + index#) * SIN(@Ray.eNormAngRads(0))+ (@Ray.xOffset * SIN(@Ray.eRotAngRads(0) + @Ray.skewRads))
            zPos1# = RadsToDeg( (@Ray.eRotAngRads(0)+ @Ray.skewRads#) )
            xImgPos1# = @Ray.eArcTotal(0)
            yImgPos1# = @Ray.r_yBegin + index#
            ...
            xPos2# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin + index#) * COS(@Ray.eNormAngRads(0)) + @Ray.eOriginX(0) + (@Ray.xOffset * COS(@Ray.eRotAngRads(0) + @Ray.skewRads))
            yPos2# = (@Ray.eRadiusNorm(0) + @Ray.r_yBegin + index#) * SIN(@Ray.eNormAngRads(0))+ (@Ray.xOffset * SIN(@Ray.eRotAngRads(0) + @Ray.skewRads))
...
SUB RunAxialScan(BYVAL RayPtr AS DWORD, BYVAL sPathPtr AS DWORD, BYVAL SCNPtr AS DWORD, BYVAL GFXPtr AS DWORD, BYVAL gWinPtrBitmap AS DWORD)
        ...
        zPos2# =  RadsToDeg(@Ray.eRotAngRads(0)) 'RadsToDeg(nRay.eRotAngRads(0))  'RadstoDeg(nRay.eRotAngRads(0))
        xImgPos2# = @Ray.eArcTotal(0)
        yImgPos2# = @Ray.r_yBegin
...
'       GRAPHIC WAITKEY$
        ...
        xCts# = 37647.14686# '1000 'counts per inch
        yCts# = 1000
        zCts# = 1000
        xImgCts# = 1000
        yImgCts# = 1000
        ...
        FOR lctr& = 0 TO @sPath.PathsGen
            ...
            IF (@sPath.xImgSeg(lCtr&)=0) AND (@sPath.yImgSeg(lCtr&)<>0) THEN       'image output on y axis only

                IF ABS(@sPath.xySegLn(lCtr&)) > ABS(@sPath.yImgSeg(lCtr&)) THEN  'physical probe movement distance > generated distance
                   pathtime# = ABS(@sPath.xySegLn(lCtr&)/yspeed#)               ' segment excursion time in seconds
                ELSE
                   pathtime# = ABS(@sPath.yImgSeg(lCtr&)/yspeed#)               ' segment excursion time in seconds
                END IF

            'combined X-Axis and Y-Axis motion moves the transducer circumferently in reference to weld
            ELSEIF (@sPath.yImgSeg(lCtr&)=0) AND (@sPath.xImgSeg(lCtr&)<>0) THEN   'image output on x axis only

                IF ABS(@sPath.xySegLn(lCtr&)) > ABS(@sPath.xImgSeg(lCtr&)) THEN        'Seg_XyL = physical surface distance
                   pathtime# = ABS(@sPath.xySegLn(lCtr&)/xspeed#)               ' segment excursion time in seconds
                ELSE
                   pathtime# = ABS(@sPath.xImgSeg(lCtr&)/xspeed#)               ' segment excursion time in seconds
                END IF
   ...
   '             WAITKEY$
            ...
            x60Hz# = @sPath.xMtrSeg(lCtr&)/n60HzSegments&
            y60Hz# = @sPath.yMtrSeg(lCtr&)/n60HzSegments&
            z60Hz# = @sPath.zMtrSeg(lCtr&)/n60HzSegments&

            xImg60Hz# = @sPath.xImgSeg(lCtr&)/n60HzSegments&
            yImg60Hz# = @sPath.yImgSeg(lCtr&)/n60HzSegments&
            ...
            FOR pctr& = 1 TO n60HzSegments&
                ...
                '*********************************************************************************************************************************
                 ...
                 IF (@sPath.yImgSeg(lCtr&)=0) AND (@sPath.xImgSeg(lCtr&)<>0) THEN 'x index
                     SLEEP 10
                 END IF
...
SUB RunCircScan(BYVAL RayPtr AS DWORD, BYVAL sPathPtr AS DWORD, BYVAL SCNPtr AS DWORD, BYVAL GFXPtr AS DWORD, BYVAL gWinPtrBitmap AS DWORD)
...
FUNCTION PBMAIN
    ...
    DIM u_GFX AS u_GFXvars

    DIM SCN AS ScanVars

    DIM u_SCN AS u_ScanVars

    DIM sPath AS ScanSegment
    ...
    GRAPHIC FONT "Times New Roman", 12, 1  'TO hFont&
'    GRAPHIC SET FONT hFont&



'!!!!!!!!!!!  - GENERATE THE NOZZLE MODEL -
'            ONLY (5) USER inputs required to generate the basic model!!!!!!!!!!!
'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    nRay.r_yBegin = 0.800# :  nRay.r_yEnd = 10.800#

    'addtional value needed if nRay.RasterAxial = False, meaning raster motion is Circ direction
    nRay.r_yIndexInc = 1.00#  'index increment for Circ Raster
...
'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    u_SCN.aProbeLen = 2.200# 'transducer length
    u_SCN.aProbeIdx = .800#   '1.00# '0.800# 'transducer index position, measured setback from front
    u_SCN.aProbeWidth = 2.600# 'transducer width

    RunAxialScan(VARPTR(nRay), VARPTR(sPath),  VARPTR(u_SCN), VARPTR(u_GFX), VARPTR(hWin(1)))
...
'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    nRay.r_yBegin = -1.60# '0'0.800# :
    nRay.r_yEnd = 0.10# '10.800#
    ...
    nRay.skewDegs = 12.00#  'transducer skew: -skewDegs = transducer on -side, UT Beam pointing CW
    nRay.xOffset = 8.00#    'transducer offset, index distance, to UT beam @ ID

    'OR:

    'nRay.skewDegs = -12.00#  'transducer skew: +skewDegs = transducer on +side, UT Beam pointing CCW
    'nRay.xOffset = -5.00#    'transducer offset, index distance, to UT beam @ ID

    GenCircSegments (VARPTR(nRay),VARPTR(sPath))
...
'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    u_SCN.cProbeLen = 2.200# 'transducer length
    u_SCN.cProbeIdx = .800#   '1.00# '0.800# 'transducer index position, measured setback from front
    ...
    u_SCN.cProbeXOffset = nRay.xOffset ' -7  '-6.20#  'Set to negative if transducer beam is pointing CW, Positive if beam is pointing CCW

    IF u_SCN.cProbeXOffset < 0 THEN
       u_SCN.cProbeSkew = ABS(nRay.skewDegs) 'opposite nRay.skew
       u_SCN.cProbeIdx = -(0.800#) 'set to (-) 'transducer index position, measured setback from front
       u_SCN.cProbeLen = -(2.200#) 'set to (-) 'probe length
    ELSE
       u_SCN.cProbeSkew = -(nRay.skewDegs) ' opposite nRay.skew
       u_SCN.cProbeIdx = (0.800#) 'set to (+) 'transducer index position, measured setback from front
       u_SCN.cProbeLen = (2.200#) 'set to (+) 'probe length
    END IF

    RunCircScan(VARPTR(nRay), VARPTR(sPath),  VARPTR(u_SCN), VARPTR(u_GFX), VARPTR(hWin(1)))
